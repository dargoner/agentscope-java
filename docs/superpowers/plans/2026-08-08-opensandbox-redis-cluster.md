...
10. 默认运行时使用官方 `sandbox-registry.cn-zhangjiakou.cr.aliyuncs.com/opensandbox/code-interpreter:v1.1.0`，生产部署可改为不可变 digest。
11. CPU/内存默认不由 Java 客户端限制，空 `resourceLimits` 交给 OpenSandbox 服务端的配额、调度和准入策略；客户端仍允许显式覆盖。
12. Java Redis 模块不实现全局 sandbox 数量、每用户配额、容量排队或抢占；服务端拒绝 create 时原样向上抛出。
13. native snapshot 与创建它的 runtime profile 绑定；镜像/入口/运行时配置不匹配时 fail closed，不静默混用新镜像和旧快照。
...
- `runtimeImage` 保存创建 sandbox 时的镜像引用；`runtimeProfileHash` 基于镜像、entrypoint 和影响运行时文件系统的配置计算，不包含 CPU/内存限额。
- 仅当当前请求的 runtime profile 与 record 一致时才能从 current/previous native snapshot 恢复。
...
private List<String> entrypoint = List.of(); // 不覆盖时由 OpenSandbox/镜像决定
private Map<String, String> resourceLimits = Map.of(); // 服务端管理资源
```

`OfficialOpenSandboxSdk.create()` 只在 entrypoint/resourceLimits 非空时调用对应 builder 方法，避免空值覆盖官方镜像和服务端策略。
